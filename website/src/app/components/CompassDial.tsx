...
export function CompassDial({ size = 360 }: { size?: number }) {
  ...
  return (
    ...
    >
      ...
      <circle cx={cx} cy={cy} r="180" fill="url(#dial-bg)" stroke="var(--ink)" strokeOpacity="0.18" strokeWidth="2" />
      <circle cx={cx} cy={cy} r="158" fill="none" stroke="var(--ink)" strokeOpacity="0.10" />
      <circle cx={cx} cy={cy} r="120" fill="none" stroke="var(--ink)" strokeOpacity="0.08" strokeDasharray="2 6" />
      ...
      >
        <polygon points={`${cx},${cy - 130} ${cx - 10},${cy} ${cx + 10},${cy}`} fill="url(#needle-n)" />
        <polygon points={`${cx},${cy + 110} ${cx - 8},${cy} ${cx + 8},${cy}`} fill="var(--ink)" opacity="0.85" />
      </g>
      <circle cx={cx} cy={cy} r="9" fill="var(--card)" stroke="var(--ink)" strokeWidth="2" />
