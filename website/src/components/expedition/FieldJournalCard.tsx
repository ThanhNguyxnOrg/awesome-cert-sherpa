...
import {AltimeterChip, type Difficulty} from './AltimeterChip';

export type FieldJournalCardProps = {
  title: string;
  vendor: string;
...
function InkStamp({date}: {date: string}): ReactNode {
  return (
    <svg
      viewBox="0 0 140 60"
      width="124"
...
/**
 * Resource / cert / feature card styled as a stitched field-journal page.
 * The hairline at the top, the perforation at the bottom, and the
 * rotated ink-stamp give it the expedition feel.
 */
export function FieldJournalCard({
    ...
    <article
      ...
      <div className="flex items-end justify-between gap-2 px-6 pb-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {difficulty && <AltimeterChip difficulty={difficulty} />}
        </div>
        {lastVerified && <InkStamp date={lastVerified} />}
      </div>
  ...
  return (
    <Link to={href} className="block no-underline" style={{textDecoration: 'none'}}>
      {content}
    </Link>
  );
}

export default FieldJournalCard;
